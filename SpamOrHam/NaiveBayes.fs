...
module Classifier =
    ...
    let score (document:TokennizedDoc) (group:DocsGroup) =
    ...
    let countIn (group:TokennizedDoc seq) (token:Token) =
        group 
        |> Seq.filter (Set.contains token) 
        |> Seq.length
    
    let analyze (group:TokennizedDoc seq)
                (totalDocs:int)
